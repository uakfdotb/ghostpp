...
void CMap :: Load( CConfig *CFG, string nCFGFile )
{
					...
					if( SFileReadFile( SubFile, SubFileData, FileLength, &BytesRead, NULL ) )
					{
						...
						if( FileFormat == 18 || FileFormat == 25 )
						{
							...
							for( uint32_t i = 0; i < RawMapNumTeams; ++i )
							{
								...
								for( unsigned char j = 0; j < m_GHost->m_MaxSlots; ++j )
								{
									if( PlayerMask & 1 )
									{
	...
	if( Slots.empty( ) )
	{
	for( uint32_t Slot = 1; Slot <= ((uint32_t) m_GHost->m_MaxSlots); ++Slot )
		{
	...
	{
		uint32_t DefaultMaxSlots = (uint32_t) m_GHost->m_MaxSlots;
		...
		while( m_Slots.size( ) < MaxSlots )
			m_Slots.push_back( CGameSlot( 0, 255, SLOTSTATUS_OPEN, 0, m_GHost->m_MaxSlots, m_GHost->m_MaxSlots, SLOTRACE_RANDOM ) );
	}
...
void CMap :: CheckValid( )
{
	...
	if( m_MapNumPlayers == 0 || m_MapNumPlayers > ((uint32_t) m_GHost->m_MaxSlots) )
	{
		m_Valid = false;
		CONSOLE_Print( "[MAP] invalid map_numplayers detected" );
	}

	if( m_MapNumTeams == 0 || m_MapNumTeams > ((uint32_t) m_GHost->m_MaxSlots) )
	{
		m_Valid = false;
		CONSOLE_Print( "[MAP] invalid map_numteams detected" );
	}

	if( m_Slots.empty( ) || m_Slots.size( ) > ((size_t) m_GHost->m_MaxSlots) )
	{
		m_Valid = false;
