...
string UTIL_FileRead( string file, uint32_t start, uint32_t length )
{
	...
	if( IS.fail( ) )
	{
		CONSOLE_Print( "[UTIL] warning - unable to read file part [%s]", file.c_str() );
		return string( );
	}
...
string UTIL_FileRead( string file )
{
	...
	if( IS.fail( ) )
	{
		CONSOLE_Print( "[UTIL] warning - unable to read file [%s]", file.c_str() );
		return string( );
	}
...
bool UTIL_FileWrite( string file, unsigned char *data, uint32_t length )
{
	...
	if( OS.fail( ) )
	{
		CONSOLE_Print( "[UTIL] warning - unable to write file [%s]", file.c_str() );
		return false;
	}
