...
bool CBaseGame :: Update( void *fd, void *send_fd )
{
	...
	if( !m_GameLoading && !m_GameLoaded && GetNumPlayers( ) < ((uint32_t) m_GHost->m_MaxSlots) )
		CreateVirtualHost( );
	...
	if( GetTime( ) - m_LastPingTime >= 5 )
	{
		...
		if( !m_CountDownStarted )
		{
			...
			else
			{
				...
				m_GHost->m_UDPSocket->Broadcast( 6112, m_Protocol->SEND_W3GS_GAMEINFO( m_GHost->m_TFT, m_GHost->m_LANWar3Version, UTIL_CreateByteArray( MapGameType, false ), m_Map->GetMapGameFlags( ), m_Map->GetMapWidth( ), m_Map->GetMapHeight( ), m_GameName, "Varlock", GetTime( ) - m_CreationTime, m_Map->GetMapPath( ), m_Map->GetMapCRC( ), (uint32_t) m_GHost->m_MaxSlots, (uint32_t) m_GHost->m_MaxSlots, m_HostPort, FixedHostCounter, m_EntryKey ) );
			}
		}
...
void CBaseGame :: EventPlayerJoined( CPotentialPlayer *potential, CIncomingJoinPlayer *joinPlayer )
{
	...
	if( GetNumPlayers( ) >= ((uint32_t) (m_GHost->m_MaxSlots-1)) || EnforcePID == m_VirtualHostPID )
		DeleteVirtualHost( );
	...
	else
	{
		...
		else
		{
			if( m_Map->GetMapFlags( ) & MAPFLAG_RANDOMRACES )
				m_Slots[SID] = CGameSlot( Player->GetPID( ), 255, SLOTSTATUS_OCCUPIED, 0, m_GHost->m_MaxSlots, m_GHost->m_MaxSlots, SLOTRACE_RANDOM );
			else
				m_Slots[SID] = CGameSlot( Player->GetPID( ), 255, SLOTSTATUS_OCCUPIED, 0, m_GHost->m_MaxSlots, m_GHost->m_MaxSlots, SLOTRACE_RANDOM | SLOTRACE_SELECTABLE );
			...
			for( unsigned char i = 0; i < m_Slots.size( ); ++i )
			{
				if( m_Slots[i].GetSlotStatus( ) == SLOTSTATUS_OCCUPIED && m_Slots[i].GetTeam( ) != m_GHost->m_MaxSlots )
					NumOtherPlayers++;
			}
...
void CBaseGame :: EventPlayerJoinedWithScore( CPotentialPlayer *potential, CIncomingJoinPlayer *joinPlayer, double score )
{
	...
	if( GetNumPlayers( ) >= ((uint32_t) (m_GHost->m_MaxSlots-1)) )
		DeleteVirtualHost( );
...
void CBaseGame :: EventPlayerChangeTeam( CGamePlayer *player, unsigned char team )
{
	...
	else
	{
		...
		else
		{
			...
			for( unsigned char i = 0; i < m_Slots.size( ); ++i )
			{
				if( m_Slots[i].GetSlotStatus( ) == SLOTSTATUS_OCCUPIED && m_Slots[i].GetTeam( ) != m_GHost->m_MaxSlots && m_Slots[i].GetPID( ) != player->GetPID( ) )
					++NumOtherPlayers;
			}
		...
		if( SID < m_Slots.size( ) )
		{
			...
			if( team == m_GHost->m_MaxSlots )
			{
				...
				m_Slots[SID].SetColour( m_GHost->m_MaxSlots );
			}
			else if( m_Slots[SID].GetColour( ) == m_GHost->m_MaxSlots )
			{
...
void CBaseGame :: EventPlayerChangeColour( CGamePlayer *player, unsigned char colour )
{
	...
	if( colour > m_GHost->m_MaxSlots-1 )
		return;
	...
	if( SID < m_Slots.size( ) )
	{
		...
		if( m_Slots[SID].GetTeam( ) == m_GHost->m_MaxSlots )
			return;
...
unsigned char CBaseGame :: GetNewColour( )
{
	...
	for( unsigned char TestColour = 0; TestColour < m_GHost->m_MaxSlots; ++TestColour )
	{
		bool InUse = false;
...
{
	if( SID < m_Slots.size( ) && colour < m_GHost->m_MaxSlots )
	{
...
void CBaseGame :: ShuffleSlots( )
{
	...
	for( vector<CGameSlot> :: iterator i = m_Slots.begin( ); i != m_Slots.end( ); ++i )
	{
		if( (*i).GetSlotStatus( ) == SLOTSTATUS_OCCUPIED && (*i).GetComputer( ) == 0 && (*i).GetTeam( ) != m_GHost->m_MaxSlots )
...
vector<unsigned char> CBaseGame :: BalanceSlotsRecursive( vector<unsigned char> PlayerIDs, unsigned char *TeamSizes, double *PlayerScores, unsigned char StartTeam )
{
	...
	for( unsigned char i = StartTeam; i < m_GHost->m_MaxSlots; ++i )
	{
		if( TeamSizes[i] > 0 )
		{
			...
			while( next_combination( PlayerIDs.begin( ), PlayerIDs.begin( ) + Mid, PlayerIDs.end( ) ) )
			{
				...
				double TeamScores[m_GHost->m_MaxSlots];

				for( unsigned char j = StartTeam; j < m_GHost->m_MaxSlots; ++j )
				{
					TeamScores[j] = 0.0;
				...
				for( unsigned char j = StartTeam; j < m_GHost->m_MaxSlots; ++j )
				{
					if( TeamSizes[j] > 0 )
					{
						for( unsigned char k = j + 1; k < m_GHost->m_MaxSlots; ++k )
						{
							if( TeamSizes[k] > 0 )
							{
...
void CBaseGame :: BalanceSlots( )
{
	...
	unsigned char TeamSizes[m_GHost->m_MaxSlots];
	double PlayerScores[13];
	memset( TeamSizes, 0, sizeof( unsigned char ) * m_GHost->m_MaxSlots );
	...
	for( unsigned char i = 0; i < m_GHost->m_MaxSlots; ++i )
	{
		if( TeamSizes[i] > 0 )
		{
			if( AlgorithmCost == 0 )
